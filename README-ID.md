...
[Readme for "Language English (default)"](./README.md)

Youtube Downloading api sederhana tidak resmi dari en.loader.to.
...
```js
// Detailnya, cek di file test.js
const yt = require("@ernestoyoofi/yt.loader-to")

// 📂 Download Media
yt.download({
  yt_link: "https://music.youtube.com/watch?v=m8Zqd58XuoA&list=OLAK5uy_n0t6KymP-tb3EUjViyWXXA4cT1raHJEl8", // Masukan url youtube video disini
  yt_format: "mp3", // Pilih beberapa format yang akan diunduh, Anda dapat melihatnya di konfigurasi script ./lib/config.js
  logs: true, // Just for check running (default: false)
  saveId: true // Menyimpan url untuk dimuat jika ada yang mengunduh dengan id url yang sama (default: false)
}).then(results => {
  console.log(results)
})
```

Hasil, Data ini muncul setelah pengunduhan selesai.

```js
// Hasil data dalam waktu 18/04/2023 (Asia/Jakarta)
...
yt.ytInfo({
  yt_link: "https://youtube.com/watch?v=4V4hl7xMBN4" // Masukan url youtube video disini
}).then(results => {
  console.log(results)
})
```

Data hasil setelah permintaan selesai.
